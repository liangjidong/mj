...
$(function() {
    ...
    function selectProductClass() {
        ...
        control.delegate('[data-product-class]', 'click', function(evt) {
            evt.preventDefault();
        ...
        for (var key in formData) {
            for (var i = 0, len = inputs.length; i < len; i++) {
                ...
                if (_this.attr('name') === key) {
                    if (key === 'webUrl' || key === 'title' || key === 'description' || key === 'webVideoUrl') {
                        var value = _this.val();
                    } else {
    ...
    function selectVideo(){ //选择视频链接
        ...
        $('#Modal .confirm').click(function(){
            ...
            formData.videoId=id;
            $('#Modal').modal('hide');
            $('.linkVideo').parents('.form-group').find('.status').html('选取成功!');
        });
    }
});
...
function uploadFile(id) {
    ...
    function beforeSend() {
        console.log("hehe = " + statusElement );
        formData[which] = 0;
        statusElement.text('上传中...');
    }

    function success(res) {
        console.log("tttttt = " + statusElement );
        console.log("res == " + res);
        var str = res;
        var len = str.indexOf("}") + 1;
        var str2 = str.substr(0,len);
        console.log("res2 == " + str2);
        ...
        if (obj.status === 0) {
            formData[which] = obj.body;
            statusElement.text('上传成功！');
        } else {
            statusElement.text(obj.message);
        }
    }
}
